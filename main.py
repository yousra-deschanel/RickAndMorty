...
YELLOW_LASER =pygame.image.load(os.path.join("assets","GREEN_laser.png"))
#background
...
class Ship:
    ...
    def shoot(self):
        if self.cool_down_counter == 0:
            laser = Laser(self.x+30, self.y-15, self.laser_img)
...
class Enemy(Ship):
    ...
    def shoot(self):
        if self.cool_down_counter == 0:
            laser = Laser(self.x-10, self.y+15, self.laser_img)
            self.lasers.append(laser)
            self.cool_down_counter = 1
